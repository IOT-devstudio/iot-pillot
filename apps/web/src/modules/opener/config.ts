...
export const CONFIG = {
   ...
   *
   * 默认 false = 进页面就直接播开屏动画，不看系统设置。
   * 之所以这样定：开屏动画是本站的入口体验本身，若某项系统设置能让它整体
   * 消失，验收和演示都会受影响，所以把「播」作为默认、「不播」作为可选项。
   *
   * 代价：对晕动症用户不够友好。若要改成尊重系统设置，把这里设为 true，
   * 行为会变成——命中 reduce 时直接展示终态，并在右上角给出
   * 「播放开屏动画」按钮供手动播放。
   *
   * 与 URL 覆盖的关系（优先级从高到低）：
   *   ?motion=force 强制播放 / ?motion=off 强制静态 > 本开关 > 系统设置
   */
